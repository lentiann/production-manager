...
def iomem_db(node):
    ...
    if node_name == "Inputs":
        ...
        for node in v:
            ...
            start = display_name.index('Text:') + 5
            node_name = display_name[start:-1]
            # print(node_name, node.get_value())
...
def filter_by_time(request):
    ...
    graph_nodes = Node.objects.filter(show_graph=True)
    for node in graph_nodes:
        node_values = node.value_set.all()
    ...
    if parameters:
        for parameter in parameters:
            if parameter.node.important and parameter.node.show_graph:
